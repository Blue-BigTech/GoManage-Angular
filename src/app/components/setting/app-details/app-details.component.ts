...
import _PRIMARY_COLOR from 'assets/color-data/primaryColors.json';
import _SECONDARY_COLOR from 'assets/color-data/secondaryColors.json';
import _TEXT_COLOR from 'assets/color-data/textColors.json';
...
export class AppDetailsComponent implements OnInit {
  ...
  SECONDARY_COLOR: any = [];
  TEXT_COLOR: any = [];
  public show_sms_setting: boolean = false;
  public txt_open_close: any = 'Off';
  public show_stripe: boolean = false;
  public txt_open_close_stripe: any = 'Off';
  constructor(
  ...
  _setColorVariable() {
    this.PRIMARY_COLOR = _PRIMARY_COLOR;
    this.SECONDARY_COLOR = _SECONDARY_COLOR;
  ...
  _updateSettingFormInit () {
    this.UPDATE_SETTING_FORM = this.fb.group({
      ...
      businessOwnerCodeVersion: ['', Validators.required],
      mobileAppCodeVersion: ['', Validators.required],
      smsClientName: ['', Validators.required],
      smsToken: ['', Validators.required],
    });
  }
  async _updateSetting () {
    this.SUCCESS_POP_UP_MESSAGE = "App detail settings updated successfully!";
    this.SUCCESS_POP_UP = true;
    this.UPDATE_SETTING_FORM.reset();
    this._updateSettingFormInit();
    this.changeDetection.detectChanges();
  }
  checkValueSmsSetting(){
    this.show_sms_setting = !this.show_sms_setting;
    if(this.show_sms_setting)  
      this.txt_open_close = "On";
    else
      this.txt_open_close = "Off";
  }
  checkValueStripe(){
    this.show_stripe = !this.show_stripe;
    if(this.show_stripe)  
      this.txt_open_close_stripe = "On";
    else
      this.txt_open_close_stripe = "Off";
  }
  ...
  _select_primary_color (color_id: any) {
    for (let color of  this.PRIMARY_COLOR){
  ...
  _select_secondary_color (color_id: any) {
    for (let color of  this.SECONDARY_COLOR){
      if (color.id == color_id){
        color.select = true;
      } else {
        color.select = false;
  ...
  _select_text_color (color_id: any) {
    for (let color of  this.TEXT_COLOR){
      if (color.id == color_id){
        color.select = true;
      } else {
        color.select = false;
